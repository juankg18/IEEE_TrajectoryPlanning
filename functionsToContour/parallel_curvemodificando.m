function [x_inner, y_inner, x_outer, y_outer, R, unv, concavity, overlap]=parallel_curvemodificando(x, y, d, make_plot, flag1, selector)
...
if nargin < 6 || isempty(selector) || ~isnumeric(selector)
    selector=0;
end
...
Cortes=1;
Premi=1;
Limi=length(x_outer(1,:));
while(i<=Limi-2)
    for j=i+2:Limi-Premi
        aja = [x_outer(1,i-1) y_outer(1,i-1); x_outer(1,i) y_outer(1,i)];
        bja = [x_outer(1,j-1) y_outer(1,j-1); x_outer(1,j) y_outer(1,j)];
        daja = diff(aja);
        dbja = diff(bja);
        maja = daja(2)/daja(1);
        mbja = dbja(2)/dbja(1);
        ...
        if (((xsol<=x_outer(1,i-1)&&xsol>=x_outer(1,i))||(xsol>=x_outer(1,i-1)&&xsol<=x_outer(1,i)))&&((xsol<=x_outer(1,j-1)&&xsol>=x_outer(1,j))||(xsol>=x_outer(1,j-1)&&xsol<=x_outer(1,j))))&&(((ysol<=y_outer(1,i-1)&&ysol>=y_outer(1,i))||(ysol>=y_outer(1,i-1)&&ysol<=y_outer(1,i)))&&((ysol<=y_outer(1,j-1)&&ysol>=y_outer(1,j))||(ysol>=y_outer(1,j-1)&&ysol<=y_outer(1,j))))
            Cortes=Cortes+1;
            x_aa=[x_outer(1,1:i-1) xsol x_outer(1,j:end)];
            y_aa=[y_outer(1,1:i-1) ysol y_outer(1,j:end)];
            x_bb=[xsol x_outer(1,i:j-1) xsol];
            y_bb=[ysol y_outer(1,i:j-1) ysol];
            MagIn=sum(sqrt(diff(x_aa).^2+diff(y_aa).^2));
            MagOut=sum(sqrt(diff(x_bb).^2+diff(y_bb).^2));
            x_outer(1,:)=NaN;
            y_outer(1,:)=NaN;
            ...
            y_outer(Cortes,:)=NaN;
            if MagIn<MagOut
                x_outer(1,1:length(x_bb))=x_bb;
                y_outer(1,1:length(y_bb))=y_bb;
                x_outer(Cortes,1:length(x_aa))=x_aa;
                y_outer(Cortes,1:length(y_aa))=y_aa;
            else
                x_outer(Cortes,1:length(x_bb))=x_bb;
                y_outer(Cortes,1:length(y_bb))=y_bb;
                x_outer(1,1:length(x_aa))=x_aa;
                y_outer(1,1:length(y_aa))=y_aa;
            end
            flagti=1;
        end
        if flagti==1
    ...
    if flagti==1
        ...
        i=2;
        Premi=1;
        Limi=length(x_outer(1,~isnan(x_outer(1,:))));
        pause(1);
    end
end
MagIn=sum(sqrt(diff(x_inner).^2+diff(y_inner).^2))
