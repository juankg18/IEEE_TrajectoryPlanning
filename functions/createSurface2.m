function [vertexR2, facesInf] = createSurface2(vertexR,facesInf,Dist,Direccion,Planar,Tolerancia,Tol2,Nk,options)
    Area=[];
    Lon=[];
    ...
    boundary = compute_boundary(facesInf, options);
    ...
    if Planar == 1
        for i=1:length(vecVex)
            ...
            NormIt1=unique(round(NormIt,Tol2),'rows');
            if size(NormIt1,1) == 2
