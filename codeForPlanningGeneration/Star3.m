...
CentPieza=[0 0 0]';
[vertex, faces, n, name] = stlRead('Star2.stl');
vertex=vertex'.*[1/2;1/2;1]/5*0.8;
faces=faces';
options.name = name;
n = size(vertex,2);
Sol=[0 0 0];
...
%toc
    ...
    for i=1:NumCapas
        Tolerancia = ((0.3-1)/(1-2)*(i-1)+0.3);
        if i==1
            [vertexAct,facesAct]=createSurface2(vertexFull(:,1:dimVtx(i),i),facesFull(:,1:dimFac(i),i),Esp/2,-1,1,Tolerancia,1,0,options);
        else
            [vertexAct,facesAct]=createSurface2(vertexFull(:,1:dimVtx(i-1),i-1),facesFull(:,1:dimFac(i-1),i-1),Esp,-1,1,Tolerancia,1,0,options);
        end
    ...
    for j=1:NumCapas
        ...
        %Contornos Paralelos
        Can=ceil(((min([max(Xentrada)-min(Xentrada) max(Yentrada)-min(Yentrada)])))/(Diametro*3.5));
        myflag = 1;
        while myflag>0
